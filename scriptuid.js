...
async function startDiagnosticFlow(bodyPart) {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:5000/api/diagnostics/questions/${bodyPart}`, {
      headers: {
        'x-auth-token': token,
      },
    });
    ...
    showCustomAlert('Error', 'Failed to load diagnostic questions. Please try again.');
    resetDiagnosticFlow();
  }
}
...
async function showResults() {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:5000/api/diagnostics/outcomes/${currentBodyPart}/${responseKey}`, {
      headers: {
        'x-auth-token': token,
      },
    });
